...
function ILocDf({ formId, resultId }) {
  ...
  const [cols, setCols] = useState("0"); // 선택 컬럼 목록(컬럼1, 컬럼2, ...)
      ...
      <div className="flex flex-row">
        <div className="flex flex-col flex-1 mr-4">
          ...
          <div ref={idxRangeRef} className={classNames(isIdxRange ? "" : "hidden", "flex flex-col")}>
            <label>
              시작 인덱스
              <input ref={idxFromRef} placeholder="미지정시 처음 인덱스" className={inputStyle} type="number" onChange={handleChange} />
            </label>
            <label>
              종료 인덱스
              <input ref={idxToRef} placeholder="미지정시 마지막 인덱스" className={inputStyle} type="number" onChange={handleChange} />
            </label>
          </div>
        </div>
        <div className="flex flex-col flex-1">
          ...
          <div ref={colDiscreteRef} className={classNames(isColRange ? "hidden" : "", "flex flex-row items-center")}>
            ...
            <MultiSelect options={colObjArray} onChange={settingCols} className="flex-1" isMulti={true} closeMenuOnSelect={false} defaultInputValue={"0"} />
          </div>
...
export default React.memo(ILocDf);
